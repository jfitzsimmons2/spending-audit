...
const handleKeydown = (e: KeyboardEvent) => {
	if (e.key === 'a') {
		...
		console.log('New category:', ct.value['Category']);
...
<template>
	<div class="flex flex-col gap-8 max-w-4xl">
		<div class="flex gap-8 justify-between items-center">
			<strong>{{ ci + 1 }} / {{ transactions.length }} transactions</strong>

			<div class="flex gap-4">
				<div>{{transactions.filter((t) => t['NecessityLevel'] === 'Necessary').length}} Necessary<br />
					{{ getTotal('Necessary') }}
				</div>
				<div>{{transactions.filter((t) => t['NecessityLevel'] === 'Necessary, but reviewable').length}} Reviewable<br />
					{{ getTotal('Necessary, but reviewable') }}</div>
				<div>{{transactions.filter((t) => t['NecessityLevel'] === 'Unnecessary').length}} Unnecessary<br />
					{{ getTotal('Unnecessary') }}</div>
			</div>
		</div>
